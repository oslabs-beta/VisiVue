...
//     // get the filePath
//     const fileName = this.getFileName(componentTree); // --> App.vue
//     // Base cases for recursion: check to see if there is a file that has been passed in by user
//     if (!fileName) {
...
//     let fileContent: string = fs.readFileSync(path.resolve(componentTree.filePath)).toString(); // --> Grabs the entire source code of the current file
//     // look at node modules for @vue/compiler-sfc for more info.
//       // just deconstructing what the parse method returns (returns an object with descriptor key). Value is type SFCDescriptor
//       // which has scriptSetup 
